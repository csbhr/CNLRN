...
def main():
    ...
    parser.add_argument('-opt', type=str, default='./options/train/train_CNLRN.yml',
                ...
                if opt['model'] in ['sr', 'srgan']:  # image restoration validation
                    if opt['dist']:
                        ...
                        for idx in range(rank, len(val_set), world_size):
                            ...
                            img_name = os.path.splitext(os.path.basename(val_data['LQ_path']))[0]
                            folder = 'valset'
                            idx_d += 1
                            if psnr_rlt.get(folder, None) is None:
                                psnr_rlt[folder] = torch.zeros(len(val_set), dtype=torch.float32,
                                                               device='cuda')
                            ...
                            # calculate PSNR
                            rlt_img, gt_img = util.crop_border([rlt_img, gt_img], opt['scale'])
